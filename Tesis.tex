...
\end{equation} donde $\rho$ es la matriz densidad del sistema, $H$ es el Hamiltoniano que modela las partes del sistema donde no hay pérdidas de energía y $L$ es el operador de Lindblat, el cual corresponde a la parte abierta del sistema. En la mecánica cuántica cerrada, el análisis se limíta al operador Hamiltoniano el cual es un operador auto adjunto. En ese caso, es suficiente resolver la ecuación de Schröedinger estacionaria:
...
\end{align} sin embargo, $\mu$ corresponde a una proyección a estados diagonales, por lo que solo sobreviven los términos que corresponden a un operador de creación y uno de aniquilación. Adicionalmente, puede verse por \cite{BarberisLC} que se cumple:

\begin{equation}
T_1 = T_2^*
\end{equation}

Los valores posibles de $\lambda_m$ son 
